...
print(f"""{PURPLE}
    ...
    ║                                                           ║
    ║          ✦  {pf('PREMIUM EDITION v4.0')}  ✦                 ║
    ║          ✦  {pf('TURBO SCANNER ENGINE')}  ✦                   ║
    ║                                                           ║
    ║          🎯  {pf('HUNT MODE: ACTIVE')}  🎯                   ║
    ║                                                           ║
    ╚═══════════════════════════════════════════════════════════╝
...
print(f'{GOLD}⚡ {pf("ULTRA CHECKER")} — {pf("TURBO MODE")} ⚡{RESET}')
print('')
...
try:
    ...
    if r.status_code == 200:
        print(f"{GREEN}  ✅ Connected! Engine online ~ ⚡{RESET}")
    else:
...
def _draw_panel(force=False):
    ...
    with _panel_lock:
        ...
        if rate_ids >= 1:
            rate_str = f"{GREEN}{rate_ids:.1f}{RESET}{DIM} /s{RESET}"
        else:
            rate_str = f"{CYAN}{rate_ids * 60:.1f}{RESET}{DIM} /min{RESET}"
        
        print(f"\n{GOLD}╔{'═'*62}╗{RESET}")
        print(f"{GOLD}║{RESET}  {PINK}⚡{GOLD}⚡{PINK}⚡{RESET}  "
              f"{WHITE}{pf('ULTRA CHECKER PREMIUM')}{RESET}  "
              f"{PINK}⚡{GOLD}⚡{PINK}⚡{RESET}   "
              f"{DIM}v4.0{RESET}  {GOLD}║{RESET}")
        print(f"{GOLD}╠{'═'*62}╣{RESET}")
        
        print(f"{GOLD}║{RESET}  "
              f"{YELLOW}◆{RESET} {WHITE}HITS{RESET}       {GREEN}{hits:<6}{RESET}     "
              f"{CYAN}◆{RESET} {WHITE}GOOD{RESET}       {CYAN}{good:<6}{RESET}    "
              f"{GOLD}║{RESET}")
        print(f"{GOLD}║{RESET}  "
        ...
        print(f"{GOLD}╠{'═'*62}╣{RESET}")
        print(f"{GOLD}║{RESET}  {PINK}⚡{RESET} {WHITE}RATE{RESET}     {rate_str:<20}  "
              f"{CYAN}⏱{RESET} {WHITE}UPTIME{RESET}  {CYAN}{elapsed_fmt}{RESET}  {GOLD}║{RESET}")
        print(f"{GOLD}╠{'═'*62}╣{RESET}")
        print(f"{GOLD}║{RESET}  {DIM}┌─────────────────────────────────────────────────┐{RESET}  {GOLD}║{RESET}")
        print(f"{GOLD}║{RESET}  {DIM}│{RESET}  {PINK}✦{RESET} {WHITE}LATEST CATCHES{RESET}                              {DIM}│{RESET}  {GOLD}║{RESET}")
        print(f"{GOLD}║{RESET}  {DIM}├─────────────────────────────────────────────────┤{RESET}  {GOLD}║{RESET}")
        
        hist_list = list(_hit_hist)
        if not hist_list:
            print(f"{GOLD}║{RESET}  {DIM}│{RESET}  {DIM}hunting in progress...{RESET}{' '*28}{DIM}│{RESET}  {GOLD}║{RESET}")
        
        for entry in hist_list:
            uname, domain, followers, year, is_high = entry
            rank_icon = f"{YELLOW}★{RESET}" if is_high else f"{CYAN}▸{RESET}"
            uname_col = f"{YELLOW}" if is_high else f"{WHITE}"
            
            line = (f"  {DIM}│{RESET}  {rank_icon} {uname_col}@{uname:<14}{RESET} "
                    f"{CYAN}{domain:<9}{RESET} {GREEN}{_fmt_num(followers):<7}{RESET} "
                    f"{PINK}{year}{RESET}  {DIM}│{RESET}")
            print(f"{GOLD}║{RESET}{line}  {GOLD}║{RESET}")
        
        for _ in range(HIST_SIZE - len(hist_list)):
            print(f"{GOLD}║{RESET}  {DIM}│{RESET}{' '*49}{DIM}│{RESET}  {GOLD}║{RESET}")
        
        print(f"{GOLD}║{RESET}  {DIM}└─────────────────────────────────────────────────┘{RESET}  {GOLD}║{RESET}")
        print(f"{GOLD}╠{'═'*62}╣{RESET}")
        print(f"{GOLD}║{RESET}  {GREEN}●{RESET} {WHITE}LIVE{RESET}       "
              f"{DIM}scanned {RESET}{WHITE}{ids_checked}{RESET} {DIM}accounts{RESET}     "
              f"{YELLOW}👑 @SunrakuV2{RESET}           {GOLD}║{RESET}")
        print(f"{GOLD}╚{'═'*62}╝{RESET}")
...
<b>🗝️ {pf('RESET')}</b>      ➜  <code>{reset_mask}</code>
...
<b>💎 {pf('MONETIZE')}</b>   ➜  <b>{monetization}</b>
...
"""
        ...
        console_msg = f"""
{GOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{RESET}
  {PINK}🎯 {pf('TARGET ACQUIRED')} 🎯{RESET}
{GOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{RESET}
  {CYAN}🧬 {pf('NAME')}{RESET}       : {WHITE}{full_name}{RESET} {verify_badge}
  {CYAN}🏷️ {pf('USERNAME')}{RESET}   : {WHITE}@{username}{RESET}
  {CYAN}📬 {pf('EMAIL')}{RESET}      : {WHITE}{email}{RESET}
  {CYAN}🌍 {pf('DOMAIN')}{RESET}     : {WHITE}{domain}{RESET}
  {CYAN}🏆 {pf('FOLLOWERS')}{RESET}  : {WHITE}{followers:,}{RESET}
  {CYAN}🚀 {pf('FOLLOWING')}{RESET}  : {WHITE}{following:,}{RESET}
  {CYAN}📷 {pf('POSTS')}{RESET}      : {WHITE}{posts}{RESET}
  {CYAN}📆 {pf('YEAR')}{RESET}       : {WHITE}{year}{RESET}
  {CYAN}💭 {pf('BIO')}{RESET}        : {WHITE}{bio if bio else '-'}{RESET}
  {CYAN}🗝️ {pf('RESET')}{RESET}      : {WHITE}{reset_mask}{RESET}
  {CYAN}💎 {pf('MONETIZE')}{RESET}   : {WHITE}{monetization}{RESET}
{GOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{RESET}
  {CYAN}🔗 {pf('PROFILE')}{RESET}    : {WHITE}https://instagram.com/{username}{RESET}
{GOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{RESET}
